...
def update_project_go(project_go, current_version, next_version):
    if current_version.startswith("v"):
        old_str = current_version
    else:
        old_str = f"Version = `v{current_version}`"
    if next_version.startswith("v"):
        new_str = next_version
    else:
        new_str = f"Version = `v{next_version}`"
    with open(project_go, mode = "r", encoding = "utf-8") as f:
        src = f.read()
    txt = src.replace(old_str, new_str) 
    if src == txt:
        print("Failed, Something isn't correct could not replace", old_str, "with", new_str)
        return False
