...
// Creator returns the contents of eprint>creators>item as a struct
type Creator struct {
	XMLName xml.Name
	Given   string `xml:"creators>item>given"`
	Family  string `xml:"creators>item>family"`
}

// Subject returns a Subject code
type Subject struct {
	XMLName xml.Name
	Code    string `xml:"subjects>item"`
}
