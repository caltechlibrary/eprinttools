...
#
# init setups the connection information for harvesting EPrints content.
#
def init(collection_name, connection_string):
    global c_name, scheme, username, password, netloc
...
#
def harvest_record(base_url, key):
    global c_name
    src, err = eputil(f'{base_url}/rest/eprint/{key}.xml')
    if err != '':
        return err
    print(f'DEBUG retrieved JSON -> {src}', file = sys.stderr)
    eprint_xml_object = {}
    obj = {}
    if src != '':
    ...
    if 'eprint' in eprint_xml_object:
        if len(eprint_xml_object) > 0:
            ...
            #print(eprint_xml_object['eprint'])
            return "Can't find contents of eprint element in EPrintXML"
    else:
        #print(eprint_xml_object)
        return "Can't find eprint element in EPrintXML"
    print(f'DEBUG obj ->', obj)
    key = str(obj['eprint_id'])
    err = ''
    if dataset.has_key(c_name, key):
        print(f'DEBUG updating {key} in {c_name}', file = sys.stderr)
        err = dataset.update(c_name, key, obj)
    else:
        print(f'DEBUG creating {key} in {c_name}', file = sys.stderr)
        err = dataset.create(c_name, key, obj)
    if err != '':
        return err
    # Fetch and attach the EPrint XML
    print('DEBUG Fetch and attach EPrint XML not implemented')
    # Fetch related documents and attach
    print('DEBUG Fetch and attach documents implemented')
    return ''
...
def harvest(keys = [], start_id = 0, save_exported_keys = '', number_of_days = None, db_connection = None, include_documents = False):
    ...
    for i, key in enumerate(keys):
        ...
        if err != '':
            ...
            exported_key.append(key)
            n += 1
        bar.update(i)
    bar.finish()
    print(f'harvested {n}/{tot} harvested from {repo_name}, {e_cnt} errors')
    if save_exported_keys != '':
        print(f'saving exported keys to {save_exported_keys}')
        with open(save_exported_keys, 'w') as f:
            f.write(exported_keys.join('\n'))
